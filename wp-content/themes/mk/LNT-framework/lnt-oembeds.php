...
function mk_theme_oembed_videos() {
 ...
 $shortcode_tags = array( 'video' => $theme_shortcode_tags['video'], 'embed' => $theme_shortcode_tags['embed'] );
 ...
 $shortcode_tags = $theme_shortcode_tags; $pattern_array = array( $video_regex ); 
 ...
 $pattern = '#(' . array_reduce( $pattern_array, function ( $carry, $item ) { if ( strpos( $item, '#' ) === 0 ) { 
 // Assuming '#...#i' regexps. 
 ...
 $lines = explode( "\n", $post->post_content ); foreach ( $lines as $line ) {
 $line = trim( $line ); if ( preg_match( $pattern, $line, $matches ) ) {
 ...
 $shortcode_tags = $theme_shortcode_tags; $pattern_array = array( $audio_regex ); 
